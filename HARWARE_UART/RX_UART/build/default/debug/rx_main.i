...
# 9 "./uart.h" 2
# 25 "./uart.h"
char uart_init(unsigned char config) {

    const double frequency = 8000000 + 0.0;
    const double baudrate = 9600 + 0.0;
    long x, y, z;
    ...
    if (x > 0) {
        e1 = fabsf((frequency / (64 * (x + 1)) - baudrate) / baudrate);
        e11 = fabsf((frequency / (64 * (x + 1 + 1)) - baudrate) / baudrate);
        if (e11 < e1) {
            e1 = e11;
            x = x + 1;
        }

        e2 = fabsf((frequency / (16 * (y + 1)) - baudrate) / baudrate);
        e22 = fabsf((frequency / (16 * (y + 1 + 1)) - baudrate) / baudrate);
        if (e22 < e2) {
            e2 = e22;
            y = y + 1;
        }

        e3 = fabsf((frequency / (4 * (z + 1)) - baudrate) / baudrate);
        e33 = fabsf((frequency / (4 * (z + 1 + 1)) - baudrate) / baudrate);
        if (e33 < e3) {
            e3 = e33;
            z = z + 1;
        }

        min = fminf(e1,fminf(e2,e3));
    } else if (y > 0) {
        e2 = fabsf((frequency / (16 * (y + 1)) - baudrate) / baudrate);
        e22 = fabsf((frequency / (16 * (y + 1 + 1)) - baudrate) / baudrate);
        if (e22 < e2) e2 = e22;
...
void __attribute__((picinterrupt(("")))) low_isr(void){
    if(RCIF){
        unsigned short payload = uart_read();
        if(payload >> 1) test('x');
        lcd_char(payload);
    }
}
